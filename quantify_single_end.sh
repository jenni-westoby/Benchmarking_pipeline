...
RSEM(){
  ...
  #Find mean and sd of read lengths
  mean=`awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("total %d avg=%f stddev=%f\n",n,m,sq/n-m*m);}'  Simulation/data/simulated/$filename'.fq' | awk -F= '{print $2}' | awk '{print $1}'`
  sd=`awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("total %d avg=%f stddev=%f\n",n,m,sq/n-m*m);}' Simulation/data/simulated/$filename'.fq' | awk -F= '{print $3}'`
  ...
  ./Simulation/RSEM-1.3.0/rsem-calculate-expression --star\
		    ...
		    -p 8 \
        --fragment-length-mean $mean --fragment-length-sd $sd \
        --append-names \
...
Salmon(){
  ...
  mv Simulation/Salmon_results/Salmon_Alignment_Results/$filename/quantsorted.sf Simulation/Salmon_results/Salmon_Alignment_Results/$filename/quant.sf
	...
	Simulation/Salmon-0.7.2_linux_x86_64/bin/salmon --no-version-check quant -i Simulation/indices/Salmon_SMEM/transcripts_index_SMEM -l A -r Simulation/data/simulated/$filename'.fq' -o Simulation/Salmon_results/Salmon_SMEM_results/$filename -p 8
  ...
  mv Simulation/Salmon_results/Salmon_SMEM_results/$filename/quantsorted.sf Simulation/Salmon_results/Salmon_SMEM_results/$filename/quant.sf
	...
	Simulation/Salmon-0.7.2_linux_x86_64/bin/salmon --no-version-check quant -i Simulation/indices/Salmon_quasi/transcripts_index_quasi -l A -r Simulation/data/simulated/$filename'.fq' -o Simulation/Salmon_results/Salmon_quasi_results/$filename -p 8
...
eXpress () {
  ...
  sd=`awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("total %d avg=%f stddev=%f\n",n,m,sq/n-m*m);}' Simulation/data/simulated/$filename'.fq' | awk -F= '{print $3}'`
	...
	#Run eXpress
	./Simulation/express-1.5.1-linux_x86_64/express Simulation/ref/reference.transcripts.fa Simulation/bamfiles/simulated/$filename'Aligned.toTranscriptome.out.bam' -o Simulation/eXpress_results/$filename -m $mean -s $sd
...
Kallisto () {
  ...
  #Find mean and standard deviation of read length
  mean=`awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("total %d avg=%f stddev=%f\n",n,m,sq/n-m*m);}'  Simulation/data/simulated/$filename'.fq' | awk -F= '{print $2}' | awk '{print $1}'`
  sd=`awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("total %d avg=%f stddev=%f\n",n,m,sq/n-m*m);}' Simulation/data/simulated/$filename'.fq' | awk -F= '{print $3}'`
	...
	./Simulation/kallisto_linux-v0.43.0/kallisto quant -i Simulation/indices/Kallisto/transcripts.idx --threads=8 --output-dir=Simulation/Kallisto_results/$filename --single -l $mean -s $sd Simulation/data/simulated/$filename'.fq'
...
Sailfish(){
  ...
  ./Simulation/Sailfish-0.6.3-Linux_x86-64/bin/sailfish quant -p 8 -i Simulation/indices/Sailfish/ -l "U" {-r Simulation/data/simulated/$filename".fq"} -o Simulation/Sailfish_results/$filename
